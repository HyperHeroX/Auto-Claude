  ...
  return (
    <div className="rounded-lg border border-border bg-muted/30 p-3 space-y-3">
      <div className="flex items-center justify-between">
        <span className="text-sm font-medium text-foreground">{t('settings:infrastructure.memoryTitle')}</span>
        {isCheckingInfrastructure && (
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {infrastructureStatus?.memory.kuzuInstalled ? (
            <span className="text-xs text-success">{t('settings:infrastructure.installed')}</span>
          ) : (
            <span className="text-xs text-warning">{t('settings:infrastructure.notAvailable')}</span>
          )}
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {infrastructureStatus?.memory.databaseExists ? (
            <span className="text-xs text-success">{t('settings:infrastructure.ready')}</span>
          ) : infrastructureStatus?.memory.kuzuInstalled ? (
            <span className="text-xs text-muted-foreground">{t('settings:infrastructure.createdOnFirstUse')}</span>
          ) : (
