...
export function ImageUpload({
  ...
  const processFiles = useCallback(
    async (files: FileList | File[]) => {
      ...
      if (remainingSlots <= 0) {
        setError(t('imageUpload.maxAllowed', { max: MAX_IMAGES_PER_TASK }));
        return;
      }
      ...
      if (fileArray.length > remainingSlots) {
        setError(t('imageUpload.slotsRemaining', { count: remainingSlots }));
      }
      ...
      for (const file of filesToProcess) {
        ...
        if (!isValidImageType(file)) {
          errors.push(t('imageUpload.invalidType', { name: file.name, types: ALLOWED_IMAGE_TYPES_DISPLAY }));
          continue;
        }

        // Warn about large files
        if (file.size > MAX_IMAGE_SIZE) {
          errors.push(t('imageUpload.tooLarge', { name: file.name }));
        ...
        try {
          ...
          errors.push(t('imageUpload.processFailed', { name: file.name }));
        }
      }
      ...
      >
        ...
        <div className="space-y-1">
          <p className="text-sm font-medium text-foreground">
            {canAddMore ? t('imageUpload.dropHint') : t('imageUpload.maxReached')}
          </p>
          <p className="text-xs text-muted-foreground">
            {canAddMore
              ? t('imageUpload.formatInfo', { types: ALLOWED_IMAGE_TYPES_DISPLAY, current: images.length, max: MAX_IMAGES_PER_TASK })
              : t('imageUpload.maxInfo', { max: MAX_IMAGES_PER_TASK })}
          </p>
        </div>
      </div>
