...
export function Worktrees({ projectId }: WorktreesProps) {
  ...
  const handleMerge = async () => {
    ...
    if (!task) {
      setError(t('common:errors.taskNotFound'));
      return;
    }
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        setMergeResult({
          success: false,
          message: result.error || t('common:errors.mergeFailed')
        });
      }
    } catch (err) {
      setMergeResult({
        success: false,
        message: err instanceof Error ? err.message : t('common:errors.mergeFailed')
      });
    } finally {
      setIsMerging(false);
    }
              ...
              <div className="rounded-lg bg-muted p-4 text-sm space-y-3">
                ...
                <div className="border-t border-border pt-3 mt-3">
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span>
                      {t('common:worktrees.commitsAndFiles', { commits: selectedWorktree.commitCount ?? 0, files: selectedWorktree.filesChanged ?? 0 })}
                    </span>
                  </div>
                </div>
              </div>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              {t('common:worktrees.cannotBeUndone')}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
