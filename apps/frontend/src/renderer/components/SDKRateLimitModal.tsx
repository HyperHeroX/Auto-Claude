...
function getSourceName(source: SDKRateLimitInfo['source'], t: (key: string) => string): string {
  switch (source) {
    case 'changelog': return t('rateLimit.sdk.sources.changelog');
    case 'task': return t('rateLimit.sdk.sources.task');
    case 'roadmap': return t('rateLimit.sdk.sources.roadmap');
    case 'ideation': return t('rateLimit.sdk.sources.ideation');
    case 'title-generator': return t('rateLimit.sdk.sources.titleGenerator');
    default: return t('rateLimit.sdk.sources.default');
  }
}
              ...
              <>
                <p className="text-sm text-muted-foreground mb-3">
                  {suggestedProfile ? (
                    ...
                    t('rateLimit.sdk.switchAndRetryDesc')
                  )}
                </p>
                        ...
                        >
                          <span className="truncate flex items-center gap-2">
                            ...
                            {profile.id === sdkRateLimitInfo.suggestedProfile?.id && (
                              <Star className="h-3 w-3 text-yellow-500" aria-label={t('rateLimit.sdk.recommendedLabel')} />
                            )}
                          </span>
