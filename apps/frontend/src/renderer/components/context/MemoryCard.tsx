...
export function MemoryCard({ memory }: MemoryCardProps) {
  ...
  const sessionLabel = memory.session_number
    ? t('memory.sessionLabel', { number: memory.session_number })
    : parsed?.session_number
      ? t('memory.sessionLabel', { number: parsed.session_number })
      : null;
        ...
        {expanded && parsed && (
          <div className="mt-4 space-y-4 pt-4 border-t border-border/50">
            ...
            {parsed.what_worked && parsed.what_worked.length > 0 && (
              <div>
                <SectionHeader icon={CheckCircle2} title={t('memory.whatWorked')} count={parsed.what_worked.length} />
            ...
            {parsed.what_failed && parsed.what_failed.length > 0 && (
              <div>
                <SectionHeader icon={XCircle} title={t('memory.whatFailed')} count={parsed.what_failed.length} />
                <ul className="space-y-0.5">
            ...
            {((parsed.recommendations_for_next_session?.length ?? 0) > 0 ||
              ...
              <div>
                <SectionHeader
                  icon={Lightbulb}
                  title={t('memory.recommendations')}
            ...
            {parsed.discoveries?.patterns_discovered && parsed.discoveries.patterns_discovered.length > 0 && (
              <div>
                <SectionHeader icon={Sparkles} title={t('memory.patterns')} count={parsed.discoveries.patterns_discovered.length} />
            ...
            {parsed.discoveries?.gotchas_discovered && parsed.discoveries.gotchas_discovered.length > 0 && (
              <div>
                <SectionHeader icon={AlertTriangle} title={t('memory.gotchas')} count={parsed.discoveries.gotchas_discovered.length} />
                <ul className="space-y-0.5">
            ...
            {parsed.discoveries?.changed_files && parsed.discoveries.changed_files.length > 0 && (
              <div>
                <SectionHeader icon={FileCode} title={t('memory.changedFiles')} count={parsed.discoveries.changed_files.length} />
            ...
            {parsed.discoveries?.file_insights && parsed.discoveries.file_insights.length > 0 && (
              <div>
                <SectionHeader icon={FileCode} title={t('memory.fileInsights')} count={parsed.discoveries.file_insights.length} />
            ...
            {parsed.subtasks_completed && parsed.subtasks_completed.length > 0 && (
              <div>
                <SectionHeader icon={CheckCircle2} title={t('memory.subtasksCompleted')} count={parsed.subtasks_completed.length} />
                <div className="flex flex-wrap gap-1.5 pl-4">
