    ...
    <div className="h-full w-full overflow-y-auto overflow-x-hidden p-4 space-y-3">
      {task.subtasks.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-sm font-medium text-muted-foreground mb-1">{t('tasks:subtasks.noSubtasks')}</p>
          <p className="text-xs text-muted-foreground/70">
            {t('tasks:subtasks.afterPlanning')}
          </p>
        </div>
      ) : (
        <>
          ...
          <div className="flex items-center justify-between text-xs text-muted-foreground pb-2 border-b border-border/50">
            <span>{t('tasks:subtasks.completedCount', { completed: task.subtasks.filter(c => c.status === 'completed').length, total: task.subtasks.length })}</span>
