      ...
      <AlertDialogContent className="max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
        <AlertDialogHeader>
          <AlertDialogTitle className="flex items-center gap-2">
            ...
            {t('tasks:merge.conflictsPreview')}
          </AlertDialogTitle>
          <AlertDialogDescription>
            {t('tasks:merge.conflictsDetected', { count: mergePreview?.conflicts.length || 0 })}
            {mergePreview && mergePreview.summary.autoMergeable > 0 && (
              <span className="text-success ml-1">
                {t('tasks:merge.canAutoMerge', { count: mergePreview.summary.autoMergeable })}
              </span>
            )}
          </AlertDialogDescription>
        </AlertDialogHeader>
              ...
              {mergePreview.conflicts.map((conflict, idx) => (
                ...
                >
                  ...
                  <div className="text-xs text-muted-foreground space-y-1">
                    {conflict.location && (
                      <div><span className="text-foreground/70">{t('tasks:merge.location')}</span> {conflict.location}</div>
                    )}
                    {conflict.reason && (
                      <div><span className="text-foreground/70">{t('tasks:merge.reason')}</span> {conflict.reason}</div>
                    )}
                    {conflict.strategy && (
                      <div><span className="text-foreground/70">{t('tasks:merge.strategy')}</span> {conflict.strategy}</div>
                    )}
                  </div>
                </div>
              ))}
        ...
        <AlertDialogFooter className="mt-4">
          ...
          >
            ...
            {stageOnly ? t('tasks:merge.stageWithAI') : t('tasks:merge.mergeWithAI')}
          </AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
